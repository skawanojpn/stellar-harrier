...
6. [他LLMサブエージェントとの連携](#他llmサブエージェントとの連携)
7. [開発フロー・ツール](#開発フローツール)
8. [開発方針・レビュー観点](#開発方針レビュー観点)
9. [開発履歴](#開発履歴)
10. [今後の課題](#今後の課題)
11. [次世代構想: STELLAR HARRIER 2](#次世代構想-stellar-harrier-2)
...
Claude Code（このマスタードキュメントを読んでいるエージェント）以外にも、
Gemini等の他のLLMをサブエージェントとして作業に加えるケースが今後も
出てくる想定。汎用的なルールをここに記録し、個別ツールごとの詳細指示書は
別ファイルに分離する。

### 委託してよい領域・委託してはいけない領域

- **委託してよいのは「単体の成果物として完結する、見た目/音のデータ作成」
  に限る。** 具体的には現時点で以下の3種類:
  - キャラクター/障害物の**見た目のデザイン**（`assets/characters/`配下の
    `build*Mesh`関数、1関数=1ファイルの編集用複製）
  - **BGMの作曲**（`assets/music/`配下の`STELLAR_TRACK_DATA`トラック定義、
    1曲=1ファイルの編集用複製）
  - **効果音（SE）の作曲**（`assets/sfx/`配下の`play*Sound`関数、
    1関数=1ファイルの編集用複製）
- **委託してはいけないのは「ゲームの振る舞い・仕様に関わる領域」。**
  具体的には`src/index.html`の直接編集、HP/スコア/移動パターン/攻撃仕様
  などの数値仕様、`docs/`配下の仕様書の内容、`git commit`・ビルド・
  デプロイ操作。これらは常にClaude Codeが担当する。
- 委託の可否に迷ったら、「その成果物単体をコピー&ペーストで統合できるか
  （＝関数/データ1個で完結し、他の仕様判断を含まないか）」を基準にする。

### 個別ツールの指示書は非公開・ローカル限定の別ファイルに分離する

- 各サブエージェント向けの詳細指示書（ディレクトリ構成・編集ルール・
  依頼内容一覧など）は、`docs/gemini-agent-instructions.md`のように
  **ツール名を冠した専用ファイル**として作成し、`.gitignore`に追加して
  git管理から外す（コードと無関係な内部ルーチン文書であり、依頼内容も
  頻繁に書き換わるためGitHubには公開しない）。
- 引き渡し文書（統合対象ファイルの一覧・変更内容のサマリ等、
  `docs/gemini-handover.md`のようなもの）も同様に非公開・ローカル限定とする。
- このマスタードキュメント（`docs/DEVELOPMENT.md`）や`README.md`からは、
  非公開ファイルへのリンクを張らない（リンク切れになる上、公開文書から
  非公開の運用詳細が推測できてしまうため）。「そのようなファイルが存在する」
  という事実だけをこのセクションに記録するに留める。

### 共通の作業フロー（ツールに依らない一般パターン）

1. 人間（プロジェクトオーナー）が、対象範囲とコンセプトを他LLMに具体的に指示する。
2. **既存ファイルを直接上書きさせず、まずドラフト（別ファイル）として
   新デザイン/新データを作らせる。** 他LLMの成果物が既存実装より優れている
   保証はないため、比較・差し戻しができる状態を維持する。
3. Claude Codeが、そのドラフトを人間が見て/聴いて確認できるプレビュー環境
   （Playwrightでの一時反映・スクリーンショット、Sound Test/SFX Galleryでの
   実機再生など）を用意する。`src/index.html`本体はこの段階では書き換えない。
4. 人間がプレビューを確認し、修正指示があれば他LLMがドラフトを再修正する。
   これを**人間が明示的なGoサインを出すまで**繰り返す。
5. Goサインが出たら、他LLM側が正式にドラフトの内容で既存ファイルを更新する。
6. 更新済みファイルの内容をClaude Codeに渡し、`src/index.html`への統合、
   `npm run verify`、Playwrightでの実機確認、コミット・GitHub Pagesへの
   デプロイまでをClaude Codeが行う。

Gemini向けの現在進行中の依頼内容・詳細な編集ルールは
`docs/gemini-agent-instructions.md`（非公開・ローカル限定）を参照。
